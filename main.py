...
def calculate_difference(currience, basket_price):
    ...
    if len(basket_price) >= 3:
        ...
        value_second = "{:.2f}".format(float(second_element))
        value_third = "{:.2f}".format(float(third_element) / x)
        ...
        if float(second_element) > float(first_element):
            difference1 = f"{perpetual}: {value_second} > cпот: {difference}'\n'"
        if float(second_element) < float(first_element):
            difference1 = f"Cпот: {difference} > {perpetual}: {value_second}'\n'"
